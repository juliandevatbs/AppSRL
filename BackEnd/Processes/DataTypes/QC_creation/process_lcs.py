...
def process_lcs_samples(data_to_process: dict):
    processed_data = {}
    ...
    last_lbid_proc = f"{last_lbid} LCS"
    ...
    insert_lcs_samples(processed_data)


def process_lcs_tests(data_to_process: dict):
    work_order = data_to_process.get("work_order")
    lab_sample_id = data_to_process.get("lab_sample_id")
    ...
    fixed_csid = f"{lab_sample_id} LCS"
    ...
    insert_lcs_tests(tests_mb_processed)


def process_lcs(data_to_process: dict):
    process_lcs_samples(data_to_process)
    process_lcs_tests(data_to_process)
